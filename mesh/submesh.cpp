                 ...
                 Array<int> attributes) : parent_(parent), from_(from), attributes_(attributes)
{
   ...
   else if (from == From::Boundary)
   {
      ...
      {
         const Element *pel = parent.GetBdrElement(i);

         if (SubMeshUtils::ElementHasAttribute(*pel, attributes))
         {
            Array<int> v;
            pel->GetVertices(v);
...
void SubMesh::Transfer(const GridFunction &src, GridFunction &dst)
{
   ...
   if (SubMeshUtils::IsSubMesh(src.FESpace()->GetMesh()))
   {
   ...
   else if (SubMeshUtils::IsSubMesh(dst.FESpace()->GetMesh()))
   {
