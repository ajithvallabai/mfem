...
TEST_CASE("SubMesh", "[SubMesh]")
{
    ...
    // auto fectype = GENERATE(FECType::H1);
    ...
    SECTION("3D")
    {
        ...
        out << std::setw(12) << std::setfill(separator) << transfer_str[transfer_type];
        out << std::endl;
        if (fectype == FECType::L2 && transfer_type == TransferType::SubToParent)
        {
            FAIL("Transferring from a surface SubMesh to a volume Mesh using L2 spaces is not implemented");
        }
